...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    if config.model.peft_type == "most":
        for name, param in base_model.named_parameters():
            if 'cls' in name or 'monarch' in name:
                param.requires_grad = True
                print_log(f"[Monarch] Training params: {name}", logger=logger)
            else:
                print_log(f"Frozen params: {name}", logger=logger)
